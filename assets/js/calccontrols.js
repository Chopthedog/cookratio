document.addEventListener("DOMContentLoaded", () => {
  ...
  function showCompileWarning() {
    alert("Enter at least 2 complete ingredients before continuing.");
  }

  function updateFineButtonState() {
    /*
      The button stays clickable even when there are not enough rows.
      This allows us to show the warning if the user presses DONE.
    */
  ...
  function setFinishedState(isFinished) {
    ...
    if (finished) {
      ...
      btnFine.textContent = "EDIT";
      setIngredientsFrozen(true);
    } else {
      actions.classList.add("hidden");
      btnFine.textContent = "DONE";
